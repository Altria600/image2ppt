...
def style_alignment_contract_violations(manifest, normalized_manifest=None):
    ...
    if manifest.get("schema_version") == 2 and policy != "governed":
        violations.append(
            {
                "field": "typography_policy",
                "reason": "page manifest v2 requires typography_policy: governed",
            }
        )
    elif policy_value is not None and policy != "governed":
        violations.append(
            {
                "field": "typography_policy",
