    ...
    def test_empty_image_list_is_rejected(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            with self.assertRaisesRegex(ValueError, "empty PDF"):
                synthesize_pdf([], Path(tmp) / "bundle.pdf")
    ...
    def test_images_bundle_into_one_pdf_page_per_image(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for index, size in enumerate(sizes):
                ...
                self.assertAlmostEqual(size[0], width, delta=1)
                self.assertAlmostEqual(size[1], height, delta=1)
                bitmap = page.render(scale=1.0, fill_color=(255, 255, 255, 255), draw_annots=True)
                with bitmap.to_pil() as rendered:
                    rgb = rendered.convert("RGB")
                    try:
                        center = rgb.getpixel((size[0] // 2, size[1] // 2))
                        line = rgb.getpixel((size[0] // 2, 10))
                    finally:
                        rgb.close()
                bitmap.close()
                dominant = 0 if index == 0 else 2
                self.assertGreater(center[dominant], 180)
                self.assertLess(max(center[channel] for channel in range(3) if channel != dominant), 100)
                self.assertLess(max(line), 80)
                page.close()
            document.close()

    def test_pdf_bundle_uses_high_quality_image_encoding(self):
        import tempfile
        from deck_text_hints import synthesize_pdf
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for index in range(2):
                page_dir = Path(tmp) / f"page_{index + 1:03d}"
                page_dir.mkdir()
                Image.new("RGB", (64, 36), "white").save(page_dir / "source.png")
                page_dirs.append(page_dir)

            with mock.patch.object(Image.Image, "save", autospec=True) as save:
                synthesize_pdf(page_dirs, Path(tmp) / "bundle.pdf")

            self.assertEqual([False, True], [call.kwargs["append"] for call in save.call_args_list])
            self.assertTrue(all(call.kwargs["quality"] == 100 for call in save.call_args_list))
            self.assertTrue(all(call.kwargs["subsampling"] == 0 for call in save.call_args_list))
