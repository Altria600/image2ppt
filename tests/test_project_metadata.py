...
class ProjectMetadataTests(unittest.TestCase):
    ...
    def test_secret_files_and_generated_runs_are_ignored(self) -> None:
        ...
        patterns = (".env", "config.yaml", "output/", "runs/", "__pycache__/")
    ...
    def test_page_manifest_v2_json_schema_is_valid_json(self) -> None:
        ...
        self.assertEqual(payload["$id"], "https://github.com/Altria600/image2ppt/blob/main/schemas/page-manifest-v2.schema.json")
        self.assertEqual(payload["properties"]["schema_version"]["const"], 2)
        self.assertIn("typography_policy", payload["required"])
        self.assertIn("quality_evidence", payload["required"])
        self.assertEqual(payload["properties"]["typography_policy"]["const"], "governed")
        for section in ("text_boxes", "shapes"):
            self.assertIn("text_style_id", payload["properties"][section]["items"]["properties"])
            self.assertIn("alignment_group", payload["properties"][section]["items"]["properties"])
